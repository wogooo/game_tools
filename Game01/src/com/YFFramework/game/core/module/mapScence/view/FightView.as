		...
		private function updateFight1(fightUIVo:FightUIVo):void
		{
			...
			if(effectBasicVo.skyId>0)
			{
				...
				if(positionType==TypeSkill.SkillPosition_Atk)
				{
					...
					{
						LayerManager.BgSkillLayer.playEffect(positionX,positionY,SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.floorTimeArr,false);
					}	
				...
				{  //当 为受击者 
					for each (uAtkInfo in fightUIVo.uAtkArr)    ////对所有的受击对象做 动画
					{
						...
						if(SourceCache.Instance.getRes(url))
						{
							LayerManager.SkySKillLayer.playEffect(positionX,positionY,SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.floorTimeArr,false);
						}	
						else SourceCache.Instance.loadRes(url);
					}
				}
		...
		private function playRoleAndFloorEffect(fightUIVo:FightUIVo,fightSkillBasicVo:FightSkillBasicVo,effectBasicVo:FightEffectBasicVo):void
		{
			...
			for each (uAtkInfo in fightUIVo.uAtkArr)
			{
				...
				if(effectBasicVo.uAtkBackId>0)
				{
					...
					{
						uAtk=uAtkInfo.player;
						uAtk.addBackEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.uAtkBackTimeArr,false);
					}
					else SourceCache.Instance.loadRes(url);
				}
				//被攻击者最上层特效
				if(effectBasicVo.uAtkFrontId>0)
				{
					...
					{
						uAtk=uAtkInfo.player;
						uAtk.addFrontEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.uAtkFrontTimeArr,false)
					}
					else SourceCache.Instance.loadRes(url);
				}
			}
	...
	//		print(this,"时间到达");
		...
		public function showEffect(playerView:PlayerView,skillId:int,skillLevel:int):void
		{
			...
			if(effectBasicVo.atkFrontId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					playerView.addFrontEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.atkTimeArr,false,effectBasicVo.atkTotalTimes);
				}
				else SourceCache.Instance.loadRes(url);
			}
			if(effectBasicVo.atkBackId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					playerView.addBackEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.atkTimeArr,false);
				}
				else SourceCache.Instance.loadRes(url);
			}
		}
...
//						waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
						...
						uAtk.addBackEffect(SourceCache.Instance.getRes(url) as ActionData,uAtkTimesArr,false);
					}
					else SourceCache.Instance.loadRes(url);
				}
...
//						waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
						...
						uAtk.addFrontEffect(SourceCache.Instance.getRes(url) as ActionData,uAtkTimesArr,false);
					}
					else SourceCache.Instance.loadRes(url);
			...
			if(effectBasicVo.floorId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					LayerManager.BgSkillLayer.addSpeedEffect(startX,startY,fightUIVo.mapX,fightUIVo.mapY,speed,SourceCache.Instance.getRes(url) as ActionData);
				}	
				else SourceCache.Instance.loadRes(url);
			}
			//天空层特效
			if(effectBasicVo.skyId>0)
			{
				...
				{
				//	waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
					LayerManager.SkySKillLayer.addSpeedEffect(startX,startY,fightUIVo.mapX,fightUIVo.mapY,speed,SourceCache.Instance.getRes(url) as ActionData);
				}	
				else SourceCache.Instance.loadRes(url);
			}
		}
...
//						waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
				...
				if(effectBasicVo.floorId>0)
				{
					...
					{
				//		waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
						LayerManager.BgSkillLayer.addSpeedEffect(startX,startY,endX,endY,speed,SourceCache.Instance.getRes(url) as ActionData);
					}	
					else SourceCache.Instance.loadRes(url);
				}
				//天空层特效
				if(effectBasicVo.skyId>0)
				{
					...
					{
				//		waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
						LayerManager.BgSkillLayer.addSpeedEffect(startX,startY,uAtk.roleDyVo.mapX,uAtk.roleDyVo.mapY,speed,SourceCache.Instance.getRes(url) as ActionData);
					}	
					else SourceCache.Instance.loadRes(url);
				}
			}
...
//					waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
			...
			if(effectBasicVo.floorId>0)
			{
				...
				{
			//		waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
					LayerManager.BgSkillLayer.addSuperSpeedEffect(startX,startY,endX,endY,effectBasicVo.skyTimeArr,speed,SourceCache.Instance.getRes(url) as ActionData);
				}	
				else SourceCache.Instance.loadRes(url);
			}
			//天空层特效
			if(effectBasicVo.skyId>0)
			{
				...
				{
					waitTime=(distance+0.00001)*1000/(speed*UpdateManager.IntervalRate);
				//	LayerManager.BgSkillLayer.addSpeedEffect(startX,startY,uAtk.roleDyVo.mapX,uAtk.roleDyVo.mapY,effectBasicVo.floorTotalTimes+waitTime,speed,SourceCache.Instance.getRes(url) as ActionData);
					LayerManager.BgSkillLayer.addSuperSpeedEffect(startX,startY,endX,endY,effectBasicVo.skyTimeArr,speed,SourceCache.Instance.getRes(url) as ActionData);
		...
		private function updateFight5(fightUIPtVo:FightUIPtVo):void
		{
			...
			if(effectBasicVo.skyId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					LayerManager.BgSkillLayer.playEffect(fightUIPtVo.mapX,fightUIPtVo.mapY,SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.floorTimeArr,false);
				}	
				else SourceCache.Instance.loadRes(url);
			}
...
//			}
		...
		//		DataCenter.Instance.roleSelfVo.heroState.isBreak=false;
			...
			if(effectBasicVo.atkBackId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					atk.addBackEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.atkBackTimeArr,false,TypeSkill.Skin_NoDirection,atkSKillDirection);
				}
				else SourceCache.Instance.loadRes(url);
			}
			//攻击者最上层
			if(effectBasicVo.atkFrontId>0)
			{
				...
				if(SourceCache.Instance.getRes(url))
				{
					atk.addFrontEffect(SourceCache.Instance.getRes(url) as ActionData,effectBasicVo.atkFrontTimeArr,false,effectBasicVo.atkFrontDirection,atkSKillDirection);
				}
				else SourceCache.Instance.loadRes(url);
			}
		}
