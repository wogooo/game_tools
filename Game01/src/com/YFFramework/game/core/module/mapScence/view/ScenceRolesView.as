			...
			switch(roleVo.bigCatergory)
			{
				...
				case TypeRole.BigCategory_Monster:
					...
					if(isBirth)  ///为怪物出生加上怪物特效
					{
						...
						var effectData:ActionData=SourceCache.Instance.getRes(monsterBirthUrl) as ActionData;
				//		print(this,"怪物出生");
...
//			DebugPane.Instance.log("差值"+dif);
		...
		public function updateSitCloth(playerId:String,equipBasicId:int):void
		{
			...
			///播放打坐特效
			var effectAction:ActionData=SourceCache.Instance.getRes(CommonEffectURLManager.SitEffectURL) as ActionData;
		...
		private function onEffectLoaded(e:ParamEvent):void
		{
			...
			var effectAction:ActionData=SourceCache.Instance.getRes(url) as ActionData;
