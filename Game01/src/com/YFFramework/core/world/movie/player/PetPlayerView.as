package com.YFFramework.core.world.movie.player
{
	...
	{	
		...
		{
			///判断 宠物和主人之间的距离 大于一定的距离后则需要进行宠物拉取
			var distance:Number=YFMath.distance(HeroProxy.mapX,HeroProxy.mapY,roleDyVo.mapX,roleDyVo.mapY);
			if(distance>=MaxDistance)
			{
				print(this,"拉取宠物");
				var degree:Number=DirectionUtil.getDirectionDegree(HeroProxy.direction);
				var endPoint:Point=YFMath.getLinePoint3(HeroProxy.mapX,HeroProxy.mapY,MinDistance,degree);
				//	endPoint为 地图坐标
		...
		/** 怪物发生移动  通知socket 
		 */		 
		private function noticePetMove(position:Point,path:Array,speed:int):void
		{
