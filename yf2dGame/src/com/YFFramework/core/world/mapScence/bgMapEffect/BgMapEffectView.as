package com.YFFramework.core.world.mapScence.bgMapEffect
{
	...
	import com.YFFramework.core.ui.yf2d.view.avatar.ThingEffect2DViewEx;
	...
	public class BgMapEffectView
	{
		...
		//	if(_totalDict)disposeAllChildren();
			...
			var building:ThingEffect2DView;
			var url:String;
			...
			for(var name:String in buildObj)
			{
				...
				actionData=SourceCache.Instance.getRes2(url,mapId) as YF2dActionData;
				for each (buildData in buildObj[name])
				{
					building=PoolCenter.Instance.getFromPool(ThingEffect2DView) as ThingEffect2DView;
			...
			actionData=SourceCache.Instance.getRes2(url) as YF2dActionData;
			for each(var skipData:Object in skipArr)
			{
				transferView=PoolCenter.Instance.getFromPool(ThingEffect2DViewEx) as ThingEffect2DViewEx;
		...
		private function complete(e:ParamEvent):void
		{
			...
			for each (var obj:Object in data)
			{
				...
				{
					effectView.initData(SourceCache.Instance.getRes2(url,exsitFlag) as YF2dActionData);
...
//			print(this,"资源"+url+"加载完成了");
		...
		private function disposeAllChildren():void
		{
			...
			{
				child=effectView as DisplayObject2D;
		...
		private function arangeEffectView():void
		{
			...
			var effectView:ThingEffect2DView;
			var tansferView:ThingEffect2DViewEx;
			for each (var display:DisplayObject2D in _totalDict)
			{
				effectView=display as ThingEffect2DView;
				tansferView= display as ThingEffect2DViewEx;
				if(effectView)  ///建筑特效
				{
