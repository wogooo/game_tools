...
//			_glowPlayer=null;
			...
			///硬件加速  要更新下场景
			if(!ResizeManager.Instance.isResize())
			{
				ResizeManager.Instance.setResize();
		...
		private function mouseEffectComplete(e:ParamEvent):void
		{
			var url:String=e.type;
		...
		//	var pt:Point=new Point(LayerManager.PlayerLayer.mouseX,LayerManager.PlayerLayer.mouseY);
			...
			if(!_senceRolesView.heroView.isDead)  ///当主角没有死亡
			{
				...
				{
					var myMousePt:Point=LayerManager.BgMapLayer.mousePt;
					var endX:int=myMousePt.x;
					var endY:int=myMousePt.y;
					if(!DataCenter.Instance.roleSelfVo.heroState.isLock)///当处于非战斗状态时
					{
						...
						//	if(target.roleDyVo.bigCatergory!=TypeRole.BigCategory_NPC)
							...
							if(TypeRole.CanFightAll(target.roleDyVo))	
							{
								moveCloseToPlayerForFight(target);
								print(this,"ttt:",target,target.roleDyVo.bigCatergory);
							}
		...
		public function updateMonsterDead(monsterDeadVo:MonsterDeadVo):void
		{
			...
			if(deadPlayer&&atkPlayer)
			{
				...
				if(!TypeSkill.isSpeedSkill(fightSkillBasicVo.atkType)) ///當不是運動技能時
				{
					...
					print(this,"非运动技能导致死亡");
				}
			//	else deadPlayer.pureStop();
...
//						break;
		...
		private function triggerBlinkMove(range:int=30):void
		{
			...
			if(YFMath.distance(HeroProxy.mapX,HeroProxy.mapY,bgMapMousePt.x,bgMapMousePt.y)>range)	testPoint=YFMath.getLinePoint2(HeroProxy.mapX,HeroProxy.mapY,bgMapMousePt.x,bgMapMousePt.y,range);
			else testPoint=bgMapMousePt;
		...
		private function triggerSKill(skillId:int,skillLevel:int,uAtk:PlayerView=null):void
		{
			...
			var playerLayerMousePt:Point;
			var bgMapLayerMosuePt:Point;
			if(fightSkillBasicVo.canfire())  ///该技能能够发出   过了CD时间 
			{
				...
				switch(fightSkillBasicVo.atkType)
				{
					case TypeSkill.Atk_Single_Default:
						...
						if(_senceRolesView.isCanFightPlayer(uAtk))
						{
					...
					case TypeSkill.Atk_Single_One: //单一攻击
						...
						if(!canTrigger)
						{
							 playerLayerMousePt=LayerManager.PlayerLayer.mousePt;
							_uAtkPlayer=getPlayer(playerLayerMousePt);
						}
						canTrigger=_senceRolesView.isUsablePlayer(_uAtkPlayer);
					...
					case TypeSkill.Atk_Single_Much:
						////只有当具有目标时才会播放动画  单点
						var player:PlayerView=getPlayer(LayerManager.PlayerLayer.mousePt);
						if(player)
						{
					...
					case TypeSkill.Atk_Single_MouseTarget:
						var mouseTarget:PlayerView=getPlayer(LayerManager.PlayerLayer.mousePt);
					...
					case TypeSkill.Atk_Circle_Pt_NoSpeed: /////该技能的范围为 400 也就是 点和人物之间的距离必须在400范围内
						...
						var locatePt:Point;
						bgMapLayerMosuePt=LayerManager.BgMapLayer.mousePt;
						////比较距离
						var heroToMouseDistance:int=YFMath.distance(_senceRolesView.heroView.roleDyVo.mapX,_senceRolesView.heroView.roleDyVo.mapY,bgMapLayerMosuePt.x,bgMapLayerMosuePt.y);
						if(heroToMouseDistance<distance) ////在范围内
						{
							
							rolesArr=_senceRolesView.getCircleRoles2(fightSkillBasicVo.range,bgMapLayerMosuePt.x,bgMapLayerMosuePt.y);
							...
							fightMorePtVo.mapX=bgMapLayerMosuePt.x;
							fightMorePtVo.mapY=bgMapLayerMosuePt.y;
					...
					case TypeSkill.Atk_ThreeLine_Pt_Speed:
						...
						///能够到达的目标点 
						bgMapLayerMosuePt=LayerManager.BgMapLayer.mousePt;
						rolesArr=_senceRolesView.getThreeLineRoles(bgMapLayerMosuePt.x,bgMapLayerMosuePt.y,fightSkillBasicVo.range);
						//					if(rolesArr.length>0)
						//					{
						fightMorePtVo=PoolCenter.Instance.getFromPool(FightMorePtVo) as FightMorePtVo;
						fightMorePtVo.skillId=skillId;
						fightMorePtVo.uAtkArr=rolesArr;
						fightMorePtVo.mapX=bgMapLayerMosuePt.x;
						fightMorePtVo.mapX=bgMapLayerMosuePt.y;
						noticeFightMorePt(fightMorePtVo);
						//					}
						break;
					case TypeSkill.Atk_LineMore_Pt_Speed:
						...
						bgMapLayerMosuePt=LayerManager.BgMapLayer.mousePt;
						print(this,"LayerManager.BgMapLayer.mouseX,LayerManager.BgMapLayer.mouseY",bgMapLayerMosuePt.x,bgMapLayerMosuePt.y,HeroProxy.mapX,HeroProxy.mapY);
						rolesArr=_senceRolesView.getLineRoles(bgMapLayerMosuePt.x,bgMapLayerMosuePt.y,fightSkillBasicVo.range,lineEndPt);
						fightMorePtVo=PoolCenter.Instance.getFromPool(FightMorePtVo) as FightMorePtVo;
						fightMorePtVo.skillId=skillId;
						fightMorePtVo.uAtkArr=rolesArr;
