...
//			}
		...
		//		var headData:int=buff.readInt();///读取32位整数						 --------------------------- header读取部分
				...
				//进行组包
			//	if((headData==SocketConst.HeadData)&&(buff.bytesAvailable>=bodyLen))
				...
				else _buff.position=_buff.position-SocketConst.HeadLength-SocketConst.BodyDataLen; ////继续填充缓冲区等待下一次组包
			}
