package com.notebook.utils
{
	...
	{
		private static var root:String="static/";
...
//		{
//			return getSkillDir()+"common/"+skinId+".chitu"+version;
//		}
		...
		{
			return getSkillDir()+skinId+".chitu"+version;
		}
		...
		{
			return getRoleDir()+"mountCloth/"+skinId+".chitu"+version;	
		}
		...
		{
			return getRoleDir()+"mount/"+skinId+".chitu"+version;	
		}
		/**获取打坐皮肤
		 */		
		public static function getSitCloth(skinId:int):String
		{
			return getRoleDir()+"sit/"+skinId+".chitu"+version;	
		...
		{
			return getMapDir()+skinId+".xx"+version;
		}
		/**返回地图编辑器网格数据
		 */
		...
		/**得到建筑物  所有的建筑物都是swf  name里面已经包含了 .chitu ,在地编文件.xx里面
		 */
		...
		{
			return getNpcDir()+npc+version;//+".chitu";
		}
