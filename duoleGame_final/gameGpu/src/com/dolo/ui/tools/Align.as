package com.dolo.ui.tools
{
	...
	public class Align
	{
		...
		public static var LeftBottom:String='LeftBottom';
		public static var RightBottom:String='RightBottom';
		...
		/**添加reSize事件 
		 */
		public static function addResizeEvents():void
		{
...
//				stage.addEventListener(Event.RESIZE,resetAlways);
											...
											parentHeight:Number=-1,isIntXY:Boolean=true,isResetZero:Boolean=false):void
		{
			if(isAlways){
				_index++;
				var putIndex:int = _index;
				var findValue:int = findIndex(target);
				if( findValue != -1){
					putIndex = findValue;
				}
				_always["key"+_index] = target;
...
//				stage.addEventListener(Event.RESIZE,resetAlways);
		...
		private static function findIndex(target:DisplayObject):int
		{
			for(var i:Object in _always){
				if(_always[i] == target){
					return int(String(i).slice(3));
				}
			}
			return  -1;
		}
...
//				stage.addEventListener(Event.RESIZE,resetAlways);
		 ...
		 * 
		 */
		public static function toRight(target:DisplayObject,isAlways:Boolean=false,offsetX:Number=0,offsetY:Number=0,
									   parentWidth:Number=-1,isIntXY:Boolean=true,isResetZero:Boolean=true):void
		{
			if(isAlways){
				_index++;
				_always["key"+_index] = target;
				_pars["key"+_index] = ["Right",offsetX,offsetY,parentWidth,isIntXY,isResetZero]
			}
			if( !_hasAdd ){
//				stage.addEventListener(Event.RESIZE,resetAlways);
				_hasAdd = true;
				...
				try{
					...
					switch(par[0]){
						...
						case Align.RightBottom:
							...
							break;
						case "Right":
							toRight(dis,false,par[1],par[2],par[3],par[4],par[5]);
							break;
						case "MiddleTop":
