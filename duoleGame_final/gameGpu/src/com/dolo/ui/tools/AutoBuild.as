package com.dolo.ui.tools
{
	...
	public class AutoBuild
	{
		...
		private static function buildOneDis(sp:Sprite,index:int):void
		{
			...
			switch(endName){
				case "_button":
					...
					btn.label = findLabelText(dis,index,obj);
					btn.targetSkin(dis);
				...
				case "_toggleButton":
					...
					tbtn.label = findLabelText(dis,index,obj);
					tbtn.targetSkin(dis);
					if(obj.format)tbtn.setFormat(obj.format);
					needRemove = false;
					break;
				case "_vScrollBar":
				...
				case "_checkBox":
					...
					CheckBox(controls).label = findLabelText(dis,index,obj);
					controls.targetSkin(dis);
					if(obj.format)	CheckBox(controls).setFormat(obj.format);
					needRemove = false;
					break;
				case "_radioButton":
		...
		private static function findLabelText(labelDisplayObj:DisplayObject,index:uint,obj:Object=null):String
		{
			...
			for( j=index+1;j<len;j++){
				...
				if(dis is TextField){
					if(dis.x>labelDisplayObj.x && dis.x < labelDisplayObj.x+labelDisplayObj.width/2 
						...
						if(obj)
						{
							var format:flash.text.TextFormat=TextField(dis).getTextFormat();
							obj.format=format;
						}
