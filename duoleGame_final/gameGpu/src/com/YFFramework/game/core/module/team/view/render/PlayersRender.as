	...
	public class PlayersRender extends ListRenderBase{
		...
		override protected function updateView(item:Object):void{
			...
			if(item.memberNum==0){
				...
				action_button.textField.textColor=TypeProps.COLOR_GRAY;
			}
			action_button.textField.setTextFormat(format);
			_dyId = item.dyId;
			_name = item.name;
			_iconImage.url = CharacterPointBasicManager.Instance.getTeamSmallIconURL(item.career,item.sex);
		}
