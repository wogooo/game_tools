package com.YFFramework.game.core.module.pet.view.collection
{
	...
	public class SkillsCollection{
		...
		private var _skills:Vector.<PetSkillGrid>;
		
		public function SkillsCollection(){
			_skills = new Vector.<PetSkillGrid>();
		}
		 ...
		 **/
		public function loadContent(selectable:Boolean,mc:MovieClip):void{
			if(PetDyManager.crtPetId!=-1){
				...
				for(var i:int=0;i<len;i++){
					if(skillArr[i].skillId != PetDyManager.Instance.getCrtPetDyVo().defaultSkillId){
						var skill:PetSkillGrid = new PetSkillGrid(index,false,selectable,mc["i"+index],skillArr[i].skillId,skillArr[i].skillLevel);
						_skills.push(skill);
						index++;
					}
				}
				//if(index!=0)	index--;
				for(i=index;i<skillOpenSlots;i++){
					skill= new PetSkillGrid(index,false,selectable,mc["i"+index],0,0);
					_skills.push(skill);
					index++;
				}
				for(i=skillOpenSlots;i<8;i++){
					skill = new PetSkillGrid(i,true,selectable,mc["i"+i],0,0);
					_skills.push(skill);
				}
			}
		}
		...
		public function setFilter(index:int):void{
			...
			for(var i:int=0;i<len;i++){
				_skills[i].setSelect(false);
			}
			if(index!=-1)	_skills[index].setSelect(true);
		}
		...
		public function clearSkillGrid():void{
			var len:int=_skills.length;
			for(var i:int=0;i<len;i++){
				while(_skills[i].getMC().numChildren>0){
