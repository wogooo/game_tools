package com.YFFramework.game.core.module.login.controller
{
	...
	public class ModuleLogin extends AbsModule
	{
		...
		{
			LayerManager.WindowLayer.addChild(_loginView);
			sendLogin();
		}
...
//			YFEventCenter.Instance.addEventListener(LoginEvent.C_CheckLogin,onSendSocket);
		...
		private function createHeroCallback(sLogin:SLogin):void
		{
			...
			{
				_loginView.showCreateHeroUI();
				YFEventCenter.Instance.dispatchEventWith(LoginEvent.ShowRegister);
			}
			else
			{
				...
				DataCenter.Instance.roleSelfVo.roleDyVo.tittleId=sLogin.playerInfo.titleId;
...
//				DataCenter.Instance.roleSelfVo.maxExp=sLogin.playerInfo.maxExp;
				...
				if(sLogin.playerInfo.silver != 0)
					trace("ModuleLogin110行，没有银币这个货币！")
...
//				initChatSocket();
		...
		private function sendLogin():void
		{
			if(ConfigManager.Instance.webId!="")
			{
				...
				print(this,"网站id:"+ConfigManager.Instance.webId);
