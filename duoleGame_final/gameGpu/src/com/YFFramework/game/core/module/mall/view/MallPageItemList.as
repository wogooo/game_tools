package com.YFFramework.game.core.module.mall.view
{
	...
	public class MallPageItemList extends PageItemListBase
	{
		//======================================================================
		//       public property
		//======================================================================
...
//		private var _templateId:int;
		...
		public function MallPageItemList()
		{
		...
		override protected function initItem(data:Object,view:Sprite,index:int):void
		{
			...
			_iconImg=Xdis.getChild(view,"buy_iconImage");
			...
			if(vo.binding_type == ItemType.ITEM_TYPE_EMPTY)
			{
				if(vo.item_type == ItemType.ITEM_TYPE_PROPS){
					...
					_iconImg.url = PropsBasicManager.Instance.getURL(vo.item_id);
					_itemName.text=PropsBasicManager.Instance.getPropsBasicVo(vo.item_id).name;
					Xtip.registerLinkTip(_iconImg,PropsTip,TipUtil.propsTipInitFunc,0,_propsInfo.template_id);
				}else if(vo.item_type == ItemType.ITEM_TYPE_EQUIP){
					_equipInfo=EquipBasicManager.Instance.getEquipBasicVo(vo.item_id);
					_iconImg.url=EquipBasicManager.Instance.getURL(vo.item_id);
					_itemName.text=EquipBasicManager.Instance.getEquipBasicVo(vo.item_id).name;
					Xtip.registerLinkTip(_iconImg,EquipTip,TipUtil.equipTipInitFunc,0,_equipInfo.template_id);
				}
			}
			else
			{
				if(vo.item_type == ItemType.ITEM_TYPE_PROPS){
					...
					_iconImg.url = PropsBasicManager.Instance.getURL(vo.item_id);
					_itemName.text=PropsBasicManager.Instance.getPropsBasicVo(vo.item_id).name;
					Xtip.registerLinkTip(_iconImg,PropsTip,TipUtil.propsTipInitFunc,0,_propsInfo.template_id,null,false,vo.binding_type);
				}else if(vo.item_type == ItemType.ITEM_TYPE_EQUIP){
					_equipInfo=EquipBasicManager.Instance.getEquipBasicVo(vo.item_id);
					_iconImg.url=EquipBasicManager.Instance.getURL(vo.item_id);
					_itemName.text=EquipBasicManager.Instance.getEquipBasicVo(vo.item_id).name;
					Xtip.registerLinkTip(_iconImg,EquipTip,TipUtil.equipTipInitFunc,0,_equipInfo.template_id,false,null,false,vo.binding_type);
				}
			}
