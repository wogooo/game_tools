	...
	public class PetBasicManager{
		...
		public function cacheData(jsonData:Object):void{
			for(var id:String  in jsonData){
				...
				petInfo.pet_type_name = jsonData[id].pet_type_name;
		...
		/**获取宠物静态数据模型，如果没有，返回null
		 * @param petType	宠物类型
		 * @return PetBasicVo
		 */		
		public function getPetBasicVoByType(petType:int):PetBasicVo{
			var arr:Array = _petHashMap.values();
			var len:int = arr.length;
			for(var i:int=0;i<len;i++){
				if(arr[i].pet_type==petType)	return arr[i];
			}
			return null;
		}
		 ...
		 * @return String 宠物小图标地址
		 */
		public function getHeadURL(petBasicId:int):String{
			return URLTool.getMonsterIcon((_petHashMap.get(petBasicId) as PetBasicVo).head_id);
		}
