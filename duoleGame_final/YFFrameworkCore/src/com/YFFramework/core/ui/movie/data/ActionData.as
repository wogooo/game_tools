...
package com.YFFramework.core.ui.movie.data
{
	...
	public class ActionData
	{
		...
		public function getDirectionLen(action:Number,direction:Number):Number
		{
			return Number(headerData[action][direction]["len"]);
		}
		public function getFrameRate(action:Number):Number
		{
		...
		/**得到该方向的数据信息
		 */
		public function getDirectionData(action:Number,direction:Number):Vector.<BitmapDataEx>
		{
		...
		public function getDirectionArr(action:Number):Array
		{
		 ...
		 */
		public function addActionData(addedData:ActionData,offsetAction:Number=0):void
		{
			var addedActionArr:Array=addedData.getActionArr();
			//当前的动作信息
			var currentActionArr:Array=getActionArr();
			var index:Number;
			for each (var action:Number in addedActionArr)
			{
				...
				if(index==-1)//当 本动作数组中不存在该动作    //动作 具有:  动作数组   动作信息  动作帧频      动作方向数组 动作方向长度
				{
					...
					// 动作方向长度
					for each (var direction:Number in headerData[action]["direction"] ) //动作方向数组中
					{
		...
		{
			for each (var action:Number in headerData["action"])
			{
				for each (var direction :Number in headerData[action]["direction"])
				{
		 ...
		 * 返回该数据是否可用
		 */		
		public static function isUsableActionData(actionData:ActionData):Boolean
		{
			if(actionData)
			{
