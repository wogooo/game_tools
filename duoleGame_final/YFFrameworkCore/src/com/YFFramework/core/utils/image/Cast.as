package com.YFFramework.core.utils.image
{
	...
	public class Cast
	{
		...
		{
			var w:int=Object(source).width;
			var h:int=Object(source).height;
			if(w==0)w=1;
			if(h==0)h=1;
			var bmp:BitmapData=new BitmapData(w,h,true,0x336699);
			bmp.draw(source);
			return bmp;
		}
		...
		/** 将表情转化为序列帧
		 */		
		private static function FaceMCToSequence(mc:MovieClip,pivot:Point=null,width:int=40,height:int=40):Vector.<BitmapDataEx>
		{
			var len:int=mc.totalFrames;
			var data:BitmapData;
			var dataEx:BitmapDataEx;
		...
		/**表情转化为ActionData
		 */		
		public static function FaceMCToActionData(mc:MovieClip,frameRate:int=30,pivot:Point=null,width:int=40,height:int=40):ActionData
		{
			var  arr:Vector.<BitmapDataEx>=FaceMCToSequence(mc,pivot,width,height);
			///构建actionData
