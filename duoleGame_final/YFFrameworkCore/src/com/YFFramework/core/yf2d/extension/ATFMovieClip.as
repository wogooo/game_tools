package com.YFFramework.core.yf2d.extension
{
	...
	import com.YFFramework.core.ui.yf2d.tween.TweenATFPlay;
	...
	public class ATFMovieClip extends ATFClip
	{
...
//		private var _timer:Timer;
		...
		private var _preScaleX:Number=1;
...
//			print(this,"注册scene消耗时间"+(getTimer()-t));
		...
		{
			if(_preMovieData&&_isStart)  //如果播了数据 则重新设置数据
			{
				///重新播放
		 ...
		 */		
		protected function playInit(movieData:ATFMovieData,scaleX:Number,loop:Boolean=true,useDefault:Boolean=false):void
		{
			/// 重新创建材质需要
...
//		private function completeIt(texture:Texture,movieData:ATFMovieData):void
//		{
...
//						}
					...
					else if(actionDataInjureDead.dataDict[TypeAction.Injure]) //如果没有死亡动作 但是有受击动作  
					{
						...
						if(!movieData)
						{
							movieData=getUableMovieData(actionDataInjureDead,TypeAction.Injure,direction);
						}
						if(movieData)
						{
							playInit(movieData,scaleX,loop);
		 ...
		 * @param direction
		 */
		protected function getUableMovieData(actionData:ATFActionData,action:int,direction:int):ATFMovieData
		{
			var movieData:ATFMovieData;
			if(actionData.getDirectionArr(action).length==1)  //只有一个方向
			{
				direction=actionData.getDirectionArr(action)[0];
				if(direction<=5)
				{
					scaleX=1;
				}
