package com.YFFramework.core.world.mapScence.map
{
	...
	public class BgMapView 
	{
		/**小图片的宽高
		 */		
		protected static const SliceWidth:int=512;
		protected static const SliceHeight:int=512;
		
		/** 质量差的图片    
		 */
		private var  _lowBmp:Sprite2D;
		...
		{
			_lowBmp=new Sprite2D();  ////添加低像素 图片容器
	...
	//	private function thumbLoadComplete(display:DisplayObject,data:Object=null):void 
		private function thumbLoadComplete(e:YFEvent):void
		{
		//	var content:BitmapData=(display as Bitmap).bitmapData;
			var url:String=e.type;
			var mapId:int=int(Vector.<Object>(e.param)[0]);
			var content:BitmapData=SourceCache.Instance.getRes2(url,mapId) as BitmapData;
			...
			_lowBmp.setFlashTexture(texture);
		...
		//	var isLoad:Boolean=false;
			for(var i:int=_leftIndex;i<=_rightIndex;++i)  /// x  方向
			{
				for(var j:int=_topIndex;j<=_bottomIndex;++j)  ///  y 方向
				{
					...
					{  ///当该照片存在
						if(!_highBmpContainer.contains(_highBmpDict[_indexStr]))
						{
							_highBmpContainer.addChild(_highBmpDict[_indexStr]);
							_highBmpDict[_indexStr].x=i*SliceWidth+SliceWidth*0.5;
