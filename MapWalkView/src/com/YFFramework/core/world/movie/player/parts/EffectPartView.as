	...
	//	private var _movie:BitmapMovieClip;
		...
		override public function dispose(e:Event=null):void
		{
			...
			while(numChildren)
			{
				movie=getChildAt(0) ;
				removeChild(movie);
				if(movie is BitmapMovieClipPool)BitmapMovieClipPool(movie).disposeToPool()
			}
			if(parent)parent.removeChild(this);
			_isDispose=true;
		}
		 ...
		 */		
		public function removeSelectRoleiew(display:DisplayObject):void
		{
		...
		/**添加选中光标
		 */ 
		public function addSelectRoleiew(display:DisplayObject):void
		{
