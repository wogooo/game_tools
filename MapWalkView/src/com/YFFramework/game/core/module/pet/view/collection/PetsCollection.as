package com.YFFramework.game.core.module.pet.view.collection
{
	...
	public class PetsCollection extends Sprite{
		...
		public function openSlot():void{
			for(var i:int=0;i<PetDyManager.Instance.getPetOpenSlots();i++){
				_items[i].openSlot();
			}
		}
		...
		public function loadContent():void{
			...
			for(var i:int=0;i<petArr.length;i++){
				...
				if(PetDyManager.Instance.getFightPetId()!=pet.dyId)
					_items[i].setStatus("休息");
				else
					_items[i].setStatus("出战");
				_items[i].setImg(petBasic.show_id);
				if(PetDyManager.crtPetId==pet.dyId)
					setFilter(i);
			}
			if(petArr.length==0)
				setFilter(-1);
		...
		public function addPet(pet:PetDyVo):void{
			...
			if(PetDyManager.Instance.getFightPetId()!=pet.dyId)
				petItem.setStatus("休息");
			else
				petItem.setStatus("出战");
