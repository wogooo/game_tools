package com.YFFramework.game.core.module.gameView.view
{
	...
	public class TeamMemberView extends AbsView{
		...
		public function updateMemberView():void{
			...
			IconLoader.initLoader(CharacterPointBasicManager.Instance.getTeamIconURL(memberDyVo.profession,memberDyVo.sex),_mc.iconImg);
		...
		public function shiftDown(destination:int):void{
			...
			_index = destination;
			this.addEventListener(Event.ENTER_FRAME,onDownFrame);
		}
		...
		public function shiftUpTo(destination:int):void{
			...
			_index = destination;
			this.addEventListener(Event.ENTER_FRAME,onUpFrame);
		}
		 ...
		 */		
		private function onUpFrame(e:Event):void{
			var less:Number = this.y-_toY;
			this.y -= less/speed;
			if(Math.abs(less)<1.1){
				this.y = _toY;
				this.removeEventListener(Event.ENTER_FRAME,onUpFrame);
			}
		}
		 ...
		 * @param e Event.EnterFrame
		 */	
		private function onDownFrame(e:Event):void{
			var less:Number = _toY - this.y;
			this.y += less/speed;
			if(Math.abs(less)<1.1){
				this.y = _toY;
				this.removeEventListener(Event.ENTER_FRAME,onDownFrame);
			}
		}
