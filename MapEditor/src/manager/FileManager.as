	...
	public class FileManager
	{
		...
		private function handleNpc(obj:Object,dict:Dictionary):void
		{
			...
			for  (var nameId:String in dict)
			{
				...
				data.x=movie.x;
				data.y=movie.y;
				data.url=UtilString.getFileName(movie.data.toString());
				obj.npc[nameId]=data
			}
			///如果长度为0 则进行删除
		...
		public static  function handleNpcToXML(dict:Dictionary,dir:File,npcFileName:String):void
		{
			...
			{
				movie=dict[nameId];
				childNode=<item x={movie.x} y={movie.y} url={UtilString.getFileName(String(movie.data))} id={nameId} />
...
//			var cellArr:Array;
			...
			var skipPointStr:String="mapId,mapX,mapY,left,top,right,bottom,otherMapId,otherMapX,otherMapY\r\n";
			for (var i:int=0;i!=len;++i)
			{
				...
				skipPointStr +=mapIdName+","+element.x+","+element.y+",0,0,0,0,"+mapId+","+data.x+","+data.y+"\r\n";
			}
